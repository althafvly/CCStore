...
fun CardItem(
    ...
    val baseColors = remember(card.bankName) {
        BankName.entries.find { it.displayName == card.bankName }?.gradient ?: BankName.None.gradient
    }

    // Lighten colors for dark mode
    val colors = remember(baseColors, isDark) {
        if (isDark) {
            baseColors.map { lerp(it, Color.White, 0.3f) } // blend 30% with white
        } else {
            baseColors
        }
    }
